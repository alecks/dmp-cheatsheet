...
\begin{itemize}
	...
	%
	\item Independent \(\iff p(x,y) = p_X(x) \cdot p_Y(y)\) for all \((x,y)\).
	%
...
\begin{itemize}
	...
	%
	\item Independent \(\iff f(x,y) = f_X(x) \cdot f_Y(y)\) for all \((x,y)\).
	%
...
\begin{itemize}
	...
	%
	\item \textbf{Sums of RVs}: Let \(W = X+Y\). \(f_W(w)=\int_{-\infty}^{\infty} f(x,w-x)\,dx\). If \(X\) and \(Y\) are independent, can integrate product of marginals. This is convolution: \(f_W = f_X \ast f_Y\).
	%
