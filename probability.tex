...
\begin{itemize}
	...
	%
	\item Stars and bars: dividing items between different bins. For example, choosing 3 scoops from an ice cream shop with 5 distinct flavours. \(n=5\) types and \(k=3\) items to choose. So there are \(k+(n-1)\) total positions, where \(k\) is the number of stars and \((n-1)\) is the number of bars. \(\binom{k+(n-1)}{k}\) gives us the number of unique ways to choose 3 scoops from 5 flavours (can choose the same flavour multiple times, unordered). Now imagine sharing \(5\) identical chocolates among \(3\) people. Need 2 bars to separate the people's bins, so 7 total positions. This gives us \(\binom{7}{5}\), the number of positions for the stars.
\end{itemize}
...
\begin{itemize}
	...
	%
	\item Elementary if all events have probability \(\frac{1}{|\Omega|}\).
	%
	\item \(P(F-E) = P(F \cap E^c) = P(F)-P(F \cap E)\). This is the set difference law.
	%
	\item Law of total probability: \(P(F) = P(F \cap E_1) + ... + P(F \cap E_i) = \sum_i P(F | E_i) P(E_i)\). 
	%
	\item De Morgan's: \((E \cap F)^c = E^c \cup F^c\) (flip intersection/union and take complement).
	%
	\item \(P(E|F) = \frac{P(E \cap F)}{P(F)}\). All axioms work fine, just add the `given' part to each. Rearrange if necessary.
	%
	\item Bayes' Theorem: \(P(E | F) = \frac{P(F | E)P(E)}{P(F)}\).
	%
...
\begin{itemize}
	...
	%
	\item \textbf{For IID RVs}, \(\lim_{n \to \infty} \bar{X} \sim N(n \mu, \frac{\sigma}{\sqrt{n}})\), and \(\lim_{n \to \infty} T \sim N(n \mu, \sqrt{n} \cdot \sigma)\); \textbf{asymptotically normal}.
\end{itemize}
